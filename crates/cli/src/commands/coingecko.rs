...
pub async fn coin(id: &str, fmt: OutputFormat) -> Result<()> {
    ...
    match fmt {
        ...
        OutputFormat::Table => {
            ...
            let price = md
                .and_then(|d| d.get("current_price"))
                .and_then(|p| p.get("usd"))
                .and_then(|v| v.as_f64());
            let mcap = md
                .and_then(|d| d.get("market_cap"))
                .and_then(|p| p.get("usd"))
                .and_then(|v| v.as_f64());
            let vol = md
            ...
            let chg_24h = md
                ...
                .and_then(|v| v.as_f64());
            let chg_7d = md
                .and_then(|d| d.get("price_change_percentage_7d"))
                .and_then(|v| v.as_f64());
            let chg_30d = md
                .and_then(|d| d.get("price_change_percentage_30d"))
                .and_then(|v| v.as_f64());
            let ath = md
                ...
                .and_then(|v| v.as_f64());
            let ath_chg = md
                .and_then(|d| d.get("ath_change_percentage"))
                .and_then(|p| p.get("usd"))
                .and_then(|v| v.as_f64());
            let circ = md
                .and_then(|d| d.get("circulating_supply"))
                .and_then(|v| v.as_f64());
            let total = md
                .and_then(|d| d.get("total_supply"))
                .and_then(|v| v.as_f64());
            let rank = data.get("market_cap_rank").and_then(|v| v.as_u64());
            let fdv = md
                .and_then(|d| d.get("fully_diluted_valuation"))
                .and_then(|p| p.get("usd"))
                .and_then(|v| v.as_f64());
            ...
            println!(
                "│  Market Cap    : ${:<29} │",
                mcap.map(|m| format!("{:.0}", m / 1e6).to_string() + "M")
                    .unwrap_or("—".into())
            );
            println!(
                "│  FDV           : ${:<29} │",
                fdv.map(|f| format!("{:.0}M", f / 1e6))
                    .unwrap_or("—".into())
            );
            println!(
                "│  24h Volume    : ${:<29} │",
                vol.map(|v| format!("{:.0}M", v / 1e6))
                    .unwrap_or("—".into())
            );
            println!("├─────────────────────────────────────────────────┤");
        ...
        OutputFormat::Table => {
            if let Some(pool) = data.get("data").and_then(|d| d.get("attributes")) {
                ...
                println!(
                    ...
                    name,
                    network,
                    address.get(..8).unwrap_or(address)
                );
                println!("├─────────────────────────────────────────────────┤");
                println!(
                    "│  Token Price   : ${:<29} │",
                ...
                println!(
                    "│  24h Volume    : ${:<29} │",
                    vol.map(|v| format!("{:.0}K", v / 1e3))
                        .unwrap_or("—".into())
                );
                println!(
                    "│  Liquidity     : ${:<29} │",
                    liq.map(|l| format!("{:.0}K", l / 1e3))
                        .unwrap_or("—".into())
                );
                println!("├─────────────────────────────────────────────────┤");
...
pub async fn dex_search(query: &str, fmt: OutputFormat) -> Result<()> {
    let client = backend().await?;
    let data = client
        .get("/api/coingecko/onchain/search", &[("query", query)])
        .await?;
                ...
                if !pools.is_empty() {
                    ...
                    for pool in pools.iter().take(10) {
                        ...
                        let dex = pool
                            ...
                            .and_then(|v| v.as_str())
                            .unwrap_or("?");
                        let price = pool.get("price_in_usd").and_then(|v| v.as_str());
                        println!(
                            "{:<30} {:<12} {:<15} {:>12}",
                ...
                if !tokens.is_empty() {
                    ...
                    for token in tokens.iter().take(10) {
                        ...
                        let net = token
                            ...
                            .and_then(|v| v.as_str())
                            .unwrap_or("?");
                        let price = token.get("price_in_usd").and_then(|v| v.as_str());
                        println!(
                            "{:<20} {:<8} {:<15} {:>12}",
                            &name[..name.len().min(19)],
                            sym.to_uppercase(),
    ...
    match fmt {
        ...
        OutputFormat::Table => {
            if let Some(d) = data.get("data") {
                ...
                let top_dom = d.get("top_coin_defi_dominance").and_then(|v| v.as_f64());
                ...
                println!("│  Top Protocol  : {:<30} │", top_name.unwrap_or("—"));
